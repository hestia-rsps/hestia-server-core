...
object Huffman {
    ...
    fun compress(message: String, builder: PacketBuilder) {
        try {
            ...
            compress(messageData, builder.buffer)
    ...
    /**
     * Compresses [message] using Huffman coding and writes to [buffer]
     * @param message The message to compress, split by symbol into a byte array
     * @param buffer The packet to write the compressed data too
     */
    private fun compress(message: ByteArray, buffer: ByteBuf) {
        try {
            if (HUFFMAN_MASKS == null) {
                return
            }
            var key = 0
            val startPosition = buffer.writerIndex()
            ...
            buffer.writerIndex(7 + position shr 3)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
